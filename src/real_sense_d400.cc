...
#include "rgbd_bridge/point_cloud_helpers.h"
#include "rgbd_bridge/real_sense_common.h"
#include "rgbd_bridge/scoped_singleton.h"
...
RealSenseD400::RealSenseD400(int camera_id)
  ...
  if (camera_name_.compare("Intel RealSense D415") == 0) {
    config_stream.open("cfg/d415_high_accuracy.json");
  ...
  config_.enable_stream(RS2_STREAM_COLOR, -1, 848, 480, RS2_FORMAT_RGB8, 30);
  config_.enable_stream(RS2_STREAM_DEPTH, -1, 848, 480, RS2_FORMAT_ANY, 30);
  // TODO(siyuan): supposedly high res is more accurate (because of stereo).
  // But I am not convinced this changes anything other than slowing down frame
  // rate. So I sticking with the lower res for now.
  // config.enable_stream(RS2_STREAM_DEPTH, -1, 1280, 720, RS2_FORMAT_ANY, 30);
...
void RealSenseD400::PollingThread() {
  ...
  while (run_) {
    ...
    const Intrinsics& depth_intrin = get_intrinsics(ImageType::DEPTH);
    const Intrinsics& color_intrin = get_intrinsics(ImageType::RGB);
